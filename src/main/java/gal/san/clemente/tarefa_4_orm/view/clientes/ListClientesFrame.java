package gal.san.clemente.tarefa_4_orm.view.clientes;

import gal.san.clemente.tarefa_4_orm.exception.ModelException;
import gal.san.clemente.tarefa_4_orm.model.Cliente;
import gal.san.clemente.tarefa_4_orm.view.converter.ClienteTableModel;
...
public class ListClientesFrame extends javax.swing.JFrame {
    ...
    public ListClientesFrame(IDAOManagerDB manager) throws ModelException {
        initComponents();
    ...
    private void initComponents() {
        ...
        detalle = new gal.san.clemente.tarefa_4_orm.view.clientes.DetalleClientePanel();
    ...
    private void botonEliminarClienteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonEliminarClienteActionPerformed
        if(JOptionPane.showConfirmDialog(rootPane, "Seguro que queres borralo cliente?", 
            ...
            try {
                Cliente cliente = getClienteSeleccionado();
                manager.getClienteDAO().eliminar(cliente.getId());
                obtenerDatos();
                //actualiza la tabla
                model.fireTableDataChanged();
            } catch (ModelException ex) {
    ...
    private gal.san.clemente.tarefa_4_orm.view.clientes.DetalleClientePanel detalle;
